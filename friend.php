...
<?php
$root2 = "SELECT DISTINCT *
        from flagsdb, claimsdb
            WHERE claimID = ?;
        "; // SQL with parameters
$click = $conn->prepare($root2); 
$click->bind_param("i", $claimid);
$click->execute();
$cinch = $click->get_result(); // get the mysqli result

while($a = $cinch->fetch_assoc())
  { 
         $color = '';
...
<font color = "<?php echo $color; ?>">  
      <p><b>Claim ID:</b>  <?php echo $a['claimID']; ?> </p>
      <p><b>Support Means:</b>  <?php echo $a['supportMeans']; ?> </p></font>
  <?php } //end while loop 
...
<br><u>Testimony Flags</u><br>
        ...
        <option value="AlternativeAgendas">Has agenda</option>
